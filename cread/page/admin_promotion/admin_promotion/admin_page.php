...
if(isset($_POST['add_product'])){
   ...
   if(empty($product_name) || empty($product_oldprice)  || empty($product_newprice) || empty($product_desc) || empty($product_auteur) || empty($product_dateedition) || empty($product_maisonedition) || empty($product_image) || empty($product_category)){
      ...
      $insert = "INSERT INTO promotion (promo_name, promo_desc,promo_oldprice,promo_newprice, promo_image,promo_auteur, promo_dateedition,promo_maisonedition,promo_cat) 
      VALUES('$product_name','$product_desc', '$product_oldprice',  '$product_newprice','$product_image','$product_auteur', '$product_dateedition',  '$product_maisonedition','$product_category' )";
   //   meme la 
...
if(isset($_GET['delete'])){
   $id = $_GET['delete'];
   mysqli_query($conn, "DELETE FROM promotion WHERE promo_id  = $id");
...
<div class="product-display">
   <table class="product-display-table">
      ...
      <tr>
         <td><img src="img/<?php echo $row['promo_image']; ?>" height="100" alt=""></td>
         <td><?php echo $row['promo_name']; ?></td>
         <td><?php echo $row['promo_oldprice']; ?>DA</td>
         <td><?php echo $row['promo_newprice']; ?>DA</td>
         <td>

            <a href="admin_update.php?edit=<?php echo $row['promo_id']; ?>" class="btn"> <i class="fas fa-edit"></i> edit </a>
            <a onclick="return confirm('are you sure you want to delete this product?')" href="admin_page.php?delete=<?php echo $row['promo_id']; ?>" class="btn"> <i class="fas fa-trash"></i> delete </a>
         </td>
      </tr>
   <?php } ?>
   </table>
